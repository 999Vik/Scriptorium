...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    return requireAuth(handlePOST)(req, res); // Require authentication for POST
  } else if (req.method === 'DELETE') {
    return requireAuth(handleDELETE)(req, res); // Require authentication for DELETE
  } else {
    res.setHeader('Allow', ['GET', 'POST', 'DELETE']);
    res.status(405).json({ error: `Method ${req.method} Not Allowed` });
  }
  ...
  try {
    ...
    const allBlogPosts = await prisma.blogPost.findMany({
      ...
      include: {
        author: { select: { id: true, firstName: true, lastName: true, email: true } },
        tags: true,
        templates: { select: { id: true, title: true, explanation: true, code: true } },
      },
      orderBy,
    });
...
// POST /api/blog-posts - Restricted to authenticated users
async function handlePOST(req, res) {
  try {
    const validatedData = await createBlogPostSchema.validate(req.body, { stripUnknown: true });
    ...
    const newBlogPost = await prisma.blogPost.create({
      data: {
        ...
        description,
        content,
        author: { connect: { id: req.user.id } }, // Set author as authenticated user
        tags: { connectOrCreate: tagConnectOrCreate },
        templates: { connect: templateConnect },
      },
      include: {
        author: { select: { id: true, firstName: true, lastName: true, email: true } },
        tags: true,
...
// DELETE /api/blog-posts/[id] - Restricted to author only
async function handleDELETE(req, res) {
  const { id } = req.query;
  const blogPostId = parseInt(id, 10);
